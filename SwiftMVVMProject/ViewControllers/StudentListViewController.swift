...
class StudentListViewController: UIViewController {
    ...
    @IBOutlet weak var nameTF: BindTextField!{
        didSet{
            nameTF.bind{ self.studentViewModel.name = $0 }
        }
    }
    
    @IBOutlet weak var addressTF: BindTextField!{
        didSet{
            addressTF.bind{ self.studentViewModel.address = $0 }
        }
    }
    ...
    @IBAction func addDataButton(_ sender: Any) {
        if (nameTF.text?.count ?? 0 > 0) && (addressTF.text?.count ?? 0 > 0){
            studentViewModel.addStudentData()
            clearAllTextField()
        }
        else{
            print("not Validate")
